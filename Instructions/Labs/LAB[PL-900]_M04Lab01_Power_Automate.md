...
## <a name="task-1-create-flow"></a>작업 \#1: 흐름 만들기
...
2.  **새로 만들기** 를 클릭하고 **자동화**, **클라우드 흐름** 을 선택한 다음 **자동화됨** 을 선택합니다. 그러면 새 창에서 Power Automate 흐름 편집기가 열립니다.

3. **흐름 트리거 선택** 에서 **Microsoft Dataverse** 를 검색합니다.

4. **행을 추가, 수정 또는 삭제할 때** 트리거를 선택한 다음 **만들기** 를 클릭합니다.

   * **변경 유형** 에 대해 **추가됨** 을 선택합니다.
   
   * **테이블 이름** 에 대해 **방문** 을 선택합니다.
   
   * **범위** 에 대한 **조직** 선택
   
   * 트리거 단계에서 줄임표( **...** )를 클릭하고 **이름 바꾸기** 를 클릭합니다. 이 트리거의 이름을 **"방문이 추가될 때"** 로 바꿉니다. 이것은 좋은 연습이므로 사용자 및 기타 흐름 편집자는 세부 정보에 대해 자세히 살펴볼 필요 없이 이 단계의 목적을 이해할 수 있습니다.

5. **새 단계** 를 선택합니다. 이 단계는 전자 메일을 포함하여 방문자 정보를 검색하는 데 필요합니다.

6. **Microsoft Dataverse** 를 검색합니다.

7. **ID 기준으로 행 가져오기** 작업을 선택합니다. 

   * **연락처** 를 **테이블 이름** 으로 선택합니다.
   
   * **행 ID** 필드에서 동적 콘텐츠 목록의 **방문자(값)** 를 선택합니다.
...
9. 메일을 검색하고 **Office 365 Outlook** 커넥터를 선택한 다음 **이메일 보내기(V2)** 작업을 선택합니다.
...
10.  맨 위에 있는 **제목 없음** 흐름 이름을 선택하고 `Visit notification`으로 이름을 바꿉니다.

11. **저장** 을 누릅니다.

    다음 작업에 대해 이 흐름 탭을 열어 둡니다. 흐름은 대략 다음과 같아야 합니다.

![이미지](https://user-images.githubusercontent.com/78555251/118340724-ccb13300-b4d9-11eb-96c2-c7b005bb9ac0.png)
...
14.  명령 표시줄에서 **꺼짐** 을 클릭하여 흐름을 끕니다. 이 옵션을 보려면 줄임표( **...** )를 눌러야 할 수 있습니다.

15.  이 창을 닫습니다.

# <a name="exercise-2-create-security-sweep-flow"></a>연습 #2: 보안 스윕 흐름 만들기

**목표:** 이 연습에서는 요구 사항을 구현하는 Power Automate 흐름을 만듭니다. 보안 스윕은 15분마다 실시되며 방문자가 예정된 시간을 초과하여 머물면 보안 알림이 전송됩니다.

## <a name="task-1-create-flow-to-retrieve-records"></a>작업 #1: 레코드를 검색하는 흐름을 만듭니다
...
2. **새로 만들기** 를 클릭하고 **자동화**, **클라우드 흐름** 을 선택한 다음 **예정됨** 을 선택합니다. 그러면 새 창에서 Power Automate 흐름 편집기가 열립니다.

3. **15** 분마다 반복되도록 흐름을 설정합니다.

4. **만들기** 를 클릭합니다.

5. **새 단계** 를 클릭합니다. *현재* 를 검색하고 **Microsoft Dataverse** 커넥터를 선택합니다. **행 나열** 작업을 선택합니다.

   * **테이블 이름** 으로 **방문** 을 입력합니다.
...
8.  관련 레코드에 대한 건물 데이터 검색
    ...
    * **Microsoft Dataverse** 를 선택합니다. 
    
    * **ID 기준으로 행 가져오기** 작업을 선택합니다.
    
    * **건물** 을 **테이블 이름** 으로 선택합니다.
    
    * 동적 콘텐츠 목록에서 **행 ID** 로 **건물(값)** 을 선택합니다.
...
9.  관련 레코드에 대한 방문자 데이터 검색
    ...
    * 동적 콘텐츠 목록에서 **행 ID** 로 **방문자(값)** 을 선택합니다.
    
    * **레코드 받기** 옆의 **...** 를 클릭하고 **이름 바꾸기** 를 선택합니다. **방문자 받기** 를 단계 이름으로 입력합니다.
    
10.  이메일 알림 보내기

     * 각 루프에 적용 내부의 **작업 추가** 를 클릭합니다. **Office 365 Outlook** 연결에서 **이메일 보내기(V2)** 작업을 추가합니다.

11.  전자 메일 주소를  **받는 사람** 으로 입력합니다.

12.  **제목** 필드에 다음을 입력합니다. **전체 이름** 은 **방문자 가져오기** 단계의 동적 콘텐츠입니다.

   ```
   {Full Name} overstayed their welcome
   ```
   
13.  **본문** 필드에 다음을 입력합니다. **이름** 은 **건물 받기** 단계의 동적 콘텐츠입니다. 목록의 맨 아래로 스크롤해야 할 수 있습니다.
...
14.  왼쪽 상단에서 **제목 없음** 이라는 흐름을 선택하고 **보안 스윕** 으로 이름을 바꿉니다.

15.  **저장** 을 누릅니다.

    흐름은 대략 다음과 같아야 합니다.

![보안 스윕 예약 흐름 1부](media/4-power-automate-security-sweep-flow.png)
...
2. **보안 스윕** 흐름 안이 아니라면 그 안으로 이동합니다.

3. 흐름이 열리면 **테스트** 를 클릭합니다.

4. **수동으로** 를 선택합니다.

5. **저장 및 테스트** 와 **흐름 실행** 을 클릭합니다.

6. 흐름이 완료되면 **완료** 를 클릭합니다. 

7. **각 항목에 적용** 을 펼친 다음 **전자 메일 알림 보내기** 단계를 펼칩니다. **제목** 과 **전자 메일 본문** 값을 확인합니다.

8. 뒤로 화살표를 선택하여 보안 스윕 흐름 세부 정보로 돌아갑니다. 명령 모음에서 **끄기** 를 선택합니다. 이는 테스트 시스템의 일정에 따라 흐름이 실행되지 않도록 하기 위한 것입니다.
