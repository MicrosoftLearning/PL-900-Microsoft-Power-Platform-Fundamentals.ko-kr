...
모든 사용자 지정이 완료되면 솔루션에는 여러 테이블이 포함됩니다.
...
# <a name="exercise-1-create-solution"></a>연습 \#1: 솔루션을 만드세요

## <a name="task-1-create-solution-and-publisher"></a>작업 \#1: 솔루션 및 게시자 만들기

1.  솔루션 만들기

    -   <https://make.powerapps.com>로 이동합니다. 다시 인증해야 할 수도 있습니다. **로그인** 을 클릭하고 필요한 경우 지침을 따르세요.
...
2.  게시자 만들기
    ...
    -   **게시자** 섹션에서 **+ 게시자** 를 선택합니다.

    -   표시되는 창에서 **표시 이름** 으로 **Bellows College** 를 입력합니다. 

    -   **이름** 으로 **BellowsCollege** 를 입력합니다.
...
3.  솔루션 만들기를 완료합니다.
    ...
    -   이제 **게시자** 드롭다운을 클릭하고 방금 만든 게시자인 **Bellows College** 를 선택합니다.

    -   **버전** 이 **1.0.0.0** 으로 설정되어 있는지 확인합니다. 
    
    -   **만들기** 를 클릭합니다.

# <a name="exercise-2-add-existing-and-create-new-tables"></a>연습 \#2: 기존 테이블 추가 및 새 테이블 만들기

**목표:** 이 연습에서는 솔루션에서 표준 연락처 테이블을 추가하고 건물과 방문이라는 새로운 사용자 지정 테이블을 만듭니다. 

## <a name="task-1-add-existing-table"></a>작업 \#1: 기존 테이블 추가

1.  방금 만든 **캠퍼스 관리** 솔루션을 클릭하여 엽니다.

2.  **기존 항목 추가** 를 클릭하고 **테이블** 을 선택합니다.
...
6.  **뷰** 탭을 선택하고 **활성 연락처** 뷰를 선택합니다. **추가** 를 클릭합니다.
...
10.  **추가** 를 다시 클릭합니다. 이렇게 하면 새로 만든 솔루션에 선택한 뷰 및 양식이 있는 연락처 테이블이 추가됩니다.

> 이제 솔루션에 "연락처"라는 테이블이 하나 있어야 합니다.
    
## <a name="task-2-create-building-table"></a>작업 #2: 건물 테이블 만들기

1.  브라우저에 캠퍼스 관리 솔루션이 열려 있어야 합니다. 그렇지 않은 경우 다음 단계를 수행하여 캠퍼스 관리 솔루션을 엽니다.

    * <https://make.powerapps.com>에 로그인합니다(아직 로그인하지 않은 경우).
    
    * **솔루션** 을 선택하고 방금 만든 **[사용자의 성] 캠퍼스 관리 솔루션** 을 클릭하여 엽니다.
          
2.  건물 테이블 만들기

    -   **새로 만들기** 를 클릭하고 **테이블** 을 선택합니다.
    
    -   **표시 이름** 에 **건물** 을 입력합니다. 
    
    -   **저장** 을 클릭합니다. 이렇게 하면 백그라운드에서 테이블 프로비전을 시작하는 동시에 다른 테이블 및 열을 추가할 수 있습니다.

## <a name="task-3-create-visit-table-and-columns"></a>작업 #3: 방문 테이블 및 열 만들기
...
2. 방문 테이블 만들기
   ...
   * **저장** 을 클릭합니다. 이렇게 하면 백그라운드에서 테이블 프로비전을 시작하는 동시에 다른 열을 추가할 수 있습니다.

3. 예약된 시작 열 만들기

   * **방문** 테이블을 선택합니다.

   * **열** 탭을 선택했는지 확인하고 **열 추가** 를 클릭합니다.
...
# <a name="exercise-3-create-relationships"></a>연습 \#3: 관계 만들기

**목표:** 이 연습에서는 테이블 간 관계를 추가합니다.

## <a name="task-1-create-relationships"></a>작업 #1: 관계 만들기

1.  **캠퍼스 관리** 솔루션의 **방문** 테이블이 계속 표시되어 있는지 확인합니다. 그렇지 않은 경우 이동합니다.
...
5.  왼쪽 위에 있는 **솔루션으로 돌아가기** 를 선택합니다.

6.  **모든 사용자 지정 게시** 를 선택합니다.

# <a name="exercise-4-import-data"></a>연습 \#4: 데이터 가져오기

**목표:** 이 연습에서는 샘플 데이터를 Dataverse 데이터베이스로 가져옵니다.

## <a name="task-1-import-solution"></a>작업 #1: 솔루션 가져오기

이 작업에서는 데이터 가져오기를 완료하는 데 필요한 Power Automate 흐름이 포함된 솔루션을 가져옵니다.

1. **DataImport_managed.zip** 파일이 데스크톱에 저장되어 있어야 합니다. 그렇지 않은 경우 [데이터 가져오기 솔루션](https://github.com/MicrosoftLearning/PL-900-Microsoft-Power-Platform-Fundamentals/blob/master/Allfiles/DataImport_managed.zip?raw=true)을 다운로드합니다.

2. [https://manage.visualstudio.com](<https://make.powerapps.com> ) 에 로그인합니다.
...
>
>   이 메시지는 데이터 모델이 완료되지 않았거나, 게시자 접두사가 **bc** 가 아니거나, **건물** 및 **방문** 테이블 이름이 위의 단계에 나열된 이름과 다르다는 것을 나타냅니다.

6. **다음** 을 누릅니다. 연결을 다시 설정하라는 메시지가 표시됩니다. 

7. **연결 선택** 드롭다운을 펼치고 **+ 새 연결** 을 선택합니다.

8. 새 브라우저 창 또는 탭이 열립니다. 연결을 만들라는 메시지가 표시될 때 **만들기** 를 선택합니다. 연결을 만들기 위해 필요한 경우 로그인합니다.

9. 현재 탭을 닫으면 이전의 **솔루션 가져오기** 탭으로 돌아오게 됩니다.

10. 방금 만든 연결이 선택되었는지 확인합니다. 연결이 보이지 않으면 **새로 고침** 을 클릭하여 연결 목록을 새로 고칩니다. 

11. **가져오기** 를 누릅니다.

12. 가져오기가 완료될 때까지 기다립니다.

## <a name="task-2-import-data"></a>작업 #2: 데이터 가져오기  
...
3. **상태** 가 **꺼짐** 인 경우, **데이터 가져오기** 옆의 **...** 를 선택하고 **켜기** 를 선택합니다.
   ...
   > **중요:** 오류 메시지를 수신한 경우, 만들어진 테이블 및 열이 위의 지침과 일치하는지 확인합니다.
...
## <a name="task-3-verify-data-import"></a>작업 #3: 데이터 가져오기 확인

1. 이전 Power Apps 탭으로 다시 이동합니다. 팝업에서 **완료** 를 클릭합니다. 

2. 왼쪽 탐색 표시줄에서 **솔루션** 을 선택하고 **캠퍼스 관리** 솔루션을 엽니다.

2. **방문** 테이블을 클릭하여 연 다음 **데이터** 탭을 선택합니다.

3. 오른쪽 상단 모서리에 있는 **활성 방문** 을 클릭하여 뷰 선택기를 표시한 다음 **모든 열** 을 선택합니다. 이렇게 하면 방문 데이터를 표시하는 데 사용되는 뷰가 변경됩니다. 

    > 해상도가 낮아 **활성 방문수** 가 표시되지 않는 경우, 같은 위치에 눈 모양 아이콘이 표시됩니다.

    > 가져오기에 성공하면 방문 행 목록이 표시됩니다.
