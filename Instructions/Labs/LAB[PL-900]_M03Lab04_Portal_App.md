...
**목표:** 이 연습에서는 Dataverse의 건물 목록과 일부 정적 콘텐츠를 표시하는 새 웹 페이지를 만듭니다.
...
1.  웹 페이지에 섹션 추가
    ...
    - 캔버스(웹 페이지를 표시하는 영역)에서 열이 아닌 페이지 섹션을 선택합니다.
...
4.  건물 목록을 표시하기 위한 권한을 구성합니다. 

    -   왼쪽 메뉴에서 설정(톱니 바퀴 기호)을 클릭하고 **더 많은 설정 보기**를 선택합니다. 그러면 새 탭에서 추가 설정이 열립니다.

    -   왼쪽 메뉴에서 **보안**이 표시될 때까지 아래로 스크롤하고 **테이블 사용 권한**을 선택합니다.

    -   **새로 만들기**를 클릭하고 다음 값을 추가합니다.

        -   **이름**: 건물 목록 표시
        -   **테이블 이름**: 오른쪽의 드롭다운에서 건물(bc_building)을 선택합니다.
        -   **웹 사이트**: 돋보기를 클릭하고 웹 사이트(Bellows College Visitors – [직접 입력한 이름])를 선택합니다.
        -   **액세스 유형**: 전역
        -   **권한**: 읽기
    
    -   상단 메뉴에서 **저장**을 선택합니다.
    
    -   **웹 역할** 섹션과 **기존 웹 역할 추가**가 표시될 때까지 아래로 스크롤합니다.
    
    -   돋보기를 클릭하고, **익명 사용자**를 선택하고, **추가**를 클릭합니다.
