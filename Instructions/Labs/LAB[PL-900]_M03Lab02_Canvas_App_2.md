...
2.  새 캔버스 애플리케이션 만들기
    ...
    -   캔버스 앱의 빈 창에서 앱 이름 필드에 **[사용자의 성] 캠퍼스 보안**을 입력합니다.
...
2.  텍스트 입력 개체 편집
    ...
    -   **Hint Text** 속성을 선택하고 `"Enter visitor code"` 을 값으로 입력합니다(큰따옴표 포함).
...
2. 체크 인 단추를 추가합니다.
   ...
   * 속성 창에서 단추 **텍스트** 속성을 "`Check In`" 으로 변경합니다(기존 따옴표 내에서 입력할 수 있습니다).
...
3. 체크 아웃 단추를 추가합니다.   
   ...
   * 속성 창에서 단추 **텍스트** 속성을 "`Check Out`" 으로 변경합니다(기존 따옴표 내에서 입력할 수 있습니다).
...
2. 고급 탭에서 **OnSelect** 속성을 다음 식으로 설정합니다.
   ...
   * **Patch(Visits, Visit, {'Actual Start': Now()});**. *패치* 메서드는 **방문** 변수로 식별된 행(현재 방문)인 **방문** 테이블을 업데이트합니다. 식은 *실제 시작* 열의 값을 현재 날짜 및 시간(*Now()* method)으로 설정합니다.
...
2. **아이콘 \|** 을 선택합니다.**추가**합니다. 아이콘을 선택합니다. 이 시점에서는 값이 동적이기를 원하기 때문에 어떤 아이콘을 선택하는지는 중요하지 않습니다.
