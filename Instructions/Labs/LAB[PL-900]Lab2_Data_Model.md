...
1.  Power Pages 홈 화면에서 왼쪽의 탐색 메뉴를 사용하여 **테이블**을 선택합니다.
1.  **테이블**에서 **Copilot 시작하기**를 선택하세요.
1.  **Copilot에서 빌드할 테이블 설명** 화면에 다음을 입력합니다. "*이벤트 관리를 위한 테이블을 만들어줘. 테이블에 이벤트 이름, 이벤트 데이터, 위치, 최대 참석자 수, 이벤트 세부 정보가 구분되어 표시되도록 해 줘."*
1.  **생성** 단추 옆에 있는 **테이블 설정**을 선택하고 다음과 같이 구성하세요.
    - **테이블 옵션:** 하나의 테이블
    - 관계는 포함하지 **마세요**.
...
7.  **다음으로 무엇을 하시겠어요?** 필드에 다음과 같이 입력하세요. *`Add a choice column named Event Type to Event table.`* 
1.  다음과 같은 텍스트를 입력하여 다른 열을 추가합니다.*`Add a choice column named Registration Required to Event table.`*
...
1.  맨 위의 **명령 모음**에서 **+ 기존 테이블**을 선택합니다.
1.  **검색** 필드에 **연락처**를 입력하고 **선택한 항목 추가**를 선택합니다.

    이벤트 유형에 따라 하나 이상의 세션이 있을 수 있습니다. 다양한 세션을 관리하려면 세션이 무엇인지, 세션과 연결된 이벤트가 무엇인지 정의해야 합니다. 다음으로, Copilot을 사용하여 이벤트 세션 테이블을 만듭니다.

1.  **다음으로 무엇을 하시겠어요?** 필드에 다음과 같이 입력하세요. *`Add a new table called Event Session. `* 

    Copilot은 이벤트 세션과 이벤트 발표자라는 두 개의 테이블을 만들 가능성이 높습니다. 하지만 연락처가 발표자이므로, 이벤트 발표자 테이블은 삭제할 예정입니다.

1.  필요한 경우, **다음으로 무엇을 하시겠어요?** 필드에 다음과 같이 입력하세요.*`Remove the Event Speaker table.`*
1.  Copilot에서 다음 텍스트를 입력하세요.*`Add a new text column to the Event Session table called Session Description.`*

    ![추가된 이벤트 세션 테이블을 보여 주는 스크린샷](media/546162a8b040a7bdcdcd5c3e2be44b4d.png)

    다음으로, **세션 등록**이라는 마지막 테이블을 추가하겠습니다. 이 테이블은 특정 세션에 등록하는 개인을 관리하는 데 사용됩니다.

1.  Copilot에서 다음 텍스트를 입력하세요.*`Add a new table called Session Registrations.`*
...
1.  필요한 경우, Copilot에서 다음 텍스트를 입력하세요.*`Remove the Participant table.`*
    
    경우에 따라 세션 등록 테이블에 참가자 이름 열이 추가됩니다. 나중에 데이터 모델을 저장하려고 할 때 문제가 발생할 수 있으므로 이 부분을 제거해야 합니다. (나중에 다른 참가자 열로 바꿀 예정입니다.)  

1.  필요한 경우, Copilot에서 다음 텍스트를 입력하세요.*`Remove the Participant Name column from the Session Registration table.`*

1.  이제 기본 열 필드가 보인다면, 다음 텍스트를 입력하세요.*`Rename the Primary Column to Registration Name in Session Registration table.`*

1.  Copilot에서 다음 텍스트를 입력하세요.*`Add a text column to the Session registration table called Special Instructions.`*
...
1.  레이블을 다음 항목으로 바꾸세요.
    ...
    - 점심 식사 및 학습
    - 실행
1.  **기본 선택**을 **없음**으로 설정합니다.

    ![업데이트된 이벤트 유형 선택 항목의 스크린샷 ](media/3fb723bad1c0fced1e3705124a2d4594.png)

1.  **저장** 단추를 선택합니다. (*열이 처음에 저장되지 않으면 다시 시도해 보세요.*)

    다음으로, **이벤트 세션** 테이블에 새 열을 추가하여 총 세션 등록 수를 추적합니다.

1.  왼쪽의 탐색 기능에서 **테이블**을 선택하여 **이벤트** 테이블을 종료합니다.
1.  **검색** 필드에서 **이벤트**를 입력하세요.
1.  **이벤트 세션** 테이블을 엽니다.
1.  **스키마** 제목 아래에서 **열**을 선택하세요.
1.  명령 모음에서 **새 열** 단추를 선택합니다.
1.  다음과 같이 새 열을 구성합니다.
    - **표시 이름**: `Total Registrations`
    - **데이터 형식:** 정수
    - **동작**: 롤업
...
1.  왼쪽 탐색 영역에서 **테이블**을 선택합니다.
1.  **테이블**에서 **Copilot 시작하기**를 선택하세요.
1.  **Copilot에서 빌드할 테이블 설명** 화면에 다음을 입력합니다.*`Create a table for checking out equipment. The table should include the Equipment Name, Due Date, and Item number.`*
1.  **생성** 단추 옆에 있는 **테이블 설정**을 선택하고 다음과 같이 구성하세요.
    - **테이블 옵션:** 하나의 테이블
    - 관계는 포함하지 **마세요**.
...
6.  **다음으로 무엇을 하시겠어요?** 필드에 다음과 같이 입력하세요. *`Rename the table to Equipment.`* 
1.  **다음으로 무엇을 하시겠어요?** 필드에 다음과 같이 입력하세요. *`Add a choice column named Equipment type.`* 
1.  다음과 같은 텍스트를 입력하여 다른 열을 추가합니다.*`Add a text column named Category.`*
1.  다음과 같은 텍스트를 입력하여 다른 열을 추가합니다.*`Add a text column named Status.`*

    완료된 테이블은 아래 이미지와 유사합니다.

    ![장비 테이블의 스크린샷](media/Equipment-table-checkpoint.png)

1.  테이블에 추가 열이 포함된 경우, *`Delete the [column name] column.`* 을(를) 입력하여 해당 열을 제거합니다.
1.  테이블이 이미지와 일치하면 **저장 후 종료**를 선택합니다.

## 작업 4: 장비 테이블을 직접 수정합니다.

1.  필요한 경우 웹 브라우저를 열고 Power Apps Maker Portal로 이동한 후, Microsoft 계정 자격 증명을 사용하여 로그인하세요.
1.  왼쪽 탐색 영역에서 **테이블**을 선택합니다.
1.  **검색** 필드에서 **장비**를 입력하세요.
1.  **장비** 테이블을 엽니다.
1.  **스키마** 제목 아래에서 **열**을 선택하세요.
1.  **장비 유형** 열을 찾아 여세요.
1.  필요한 경우, 레이블을 다음 항목으로 바꾸세요.
    - 전자
    - 가구
    - 도구
    - Accessories
