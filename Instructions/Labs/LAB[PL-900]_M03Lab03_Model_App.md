...
4. 데이터 환경 섹션에서 **양식** 을 선택하고 클릭하여 **기본** 양식 형식의 정보 양식을 엽니다.

**중요:** 기본적으로 모든 양식은 정보이므로 선택한 양식이 **기본** 양식 형식이고 다른 것이 아닌지 확인합니다. 기본값으로 양식에는 두 개의 필드가 있습니다. 이름(기본 필드) 및 Owner입니다.
...
1. 데이터 환경 섹션에서 **보기** 를 선택하고 클릭하여 **활성 방문** 보기를 엽니다.
...
3. 모델 기반 애플리케이션 만들기:
    ...
    0. 홈 화면의 **시작** 섹션에서 **빈 앱** 을 선택합니다.

    1. **Dataverse를 기반으로 하는 빈 앱** 에서 **만들기** 를 선택합니다.

    2. 이름에 **Bellows Campus Management** 를 입력하고 **만들기** 를 선택합니다.
...
2. 새 연락처 만들기
    ...
    2. **이름** 을 John으로 지정하고 **성** 을 Doe로 지정합니다.

    3. 개인 이메일을 **전자 메일** 에 입력합니다. 이 기능은 전자 메일을 받게 될 향후 랩에서 사용됩니다.

    4. **저장 후 닫기** 를 클릭합니다.

    5. 이제 **활성 연락처** 보기에 작성된 연락처가 표시됩니다.
...
3. 새 방문 만들기
    ...
    2. 다음과 같이 필드를 입력합니다.
        ...
        2. **예약된 시작**: 내일 날짜를 선택하고 시작 시간으로 오후 2시를 선택합니다.

        3. **예약된 종료**: 내일 날짜를 선택하고 종료 시간으로 오후 3시 30분을 선택합니다.

- **저장 후 닫기** 를 클릭합니다. 이렇게 하면 방문이 생성되고 활성 방문 보기에 표시될 것입니다.

- 보기를 **오늘의 방문** 으로 변경합니다. 내일로 예정되어 있기 때문에 더 이상 뷰에서 새 방문을 볼 수 없습니다.
